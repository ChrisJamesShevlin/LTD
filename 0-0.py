...
def calculate_insights():
    try:
        ...
        raw_targets = entries["entry_target_scores"].get().split(',')
        target_scores = []
        ...
        # --- Prepare balance & flat amount (10%) ---
        effective_balance = max(balance, 0)
        flat_amount = effective_balance * 0.10
        ...
        lam_h = time_decay(hm_rem, elapsed)
        lam_a = time_decay(aw_rem, elapsed)

        # --- Adjust for current scoreline ---
        def adjust_scoreline(h, a, lh, la, elapsed):
            d = h - a
            if d == 1:
                lh, la = lh * 0.9, la * 1.2
            elif d == -1:
        ...
        lam_h, lam_a = adjust_scoreline(home_goals, away_goals, lam_h, lam_a, elapsed)
        ...
        # --- Bayesian probabilities for win/draw/away ---
        def bayes(lam, k, r=3):
            p = r / (r + lam)
        ...
        for gh in range(6):
            for ga in range(6):
                prob = bayes(lam_h, gh) * bayes(lam_a, ga)
        ...
        total = hw + aw + dw
        if total > 0:
            hw, aw, dw = hw/total, aw/total, dw/total
        ...
        m_sum = m_h + m_d + m_a
        if m_sum > 0:
            m_h, m_d, m_a = m_h/m_sum, m_d/m_sum, m_a/m_sum
        ...
        f_sum = f_h + f_d + f_a
        if f_sum > 0:
            f_h, f_d, f_a = f_h/f_sum, f_d/f_sum, f_a/f_sum

        # --- Scoreline probability dictionary ---
        probs = {}
        for i in range(10):
            for j in range(10):
        ...
        output += (
            ...
            f"  Draw: {f_d*100:.1f}%\n"
            f"  Non-Draw: {(f_h+f_a)*100:.1f}%\n\n"
        )
        output += (
            f"Over/Under 2.5 Goals:\n"
            f"  Over: Fair {fair_odds(over_p):.2f} | Live {live_o25:.2f}\n"
            f"  Under: Fair {fair_odds(under_p):.2f} | Live {live_u25:.2f}\n\n"
        )
        output += (
        ...
        output += (
            ...
            f"  Total: {likely_h+likely_a:.2f} (Home: {likely_h:.2f}, Away: {likely_a:.2f})\n\n"
        )

        # --- Target Score Lay/Back Recommendations ---
        lays, backs = [], []
        for score, live_odds in target_scores:
            prob = probs.get(score, 0.0)
            f_od = fair_odds(prob)
            if f_od < live_odds:
                stake = flat_amount / (live_odds - 1) if live_odds > 1 else 0
                lays.append(
                    f"Lay {score[0]}-{score[1]}: Fair {f_od:.2f}, "
                    f"Live {live_odds:.2f}, Liab {flat_amount:.2f}, Stake {stake:.2f}"
                )
            elif f_od > live_odds:
                backs.append(
                    f"Back {score[0]}-{score[1]}: Fair {f_od:.2f}, "
                    f"Live {live_odds:.2f}, Stake {flat_amount:.2f}"
                )

        output += "Target Score Lay Recommendations:\n"
        if lays:
            for line in lays:
                output += f"  {line}\n"
        else:
            output += "  No lay value on targets.\n"

        output += "\nTarget Score Back Recommendations:\n"
        if backs:
            for line in backs:
                output += f"  {line}\n"
        else:
            output += "  No back value on targets.\n"
...
labels = [
    ...
    "Account Balance", "Live Under 2.5 Odds", "Live Over 2.5 Odds",
...
tk.Button(main_frame, text="Calculate Match Insights", command=calculate_insights)\
    .grid(row=len(entries)+1, column=0, columnspan=2, padx=5, pady=5)
tk.Button(main_frame, text="Reset All Fields", command=reset_all)\
    .grid(row=len(entries)+2, column=0, columnspan=2, padx=5, pady=5)
